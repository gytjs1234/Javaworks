...
public class ArraysV1 {
 ...
 public static void main(String[] args) {
	 ...
	 //저장 공간이 생성된다.
	 //intNums 배열의 10개의 저장소는 0으로 clear 된 상태로 시작
	 ...
	 intNums[3]=40;
	 intNums[4]=50;
	 //intNums[5]부터 intNㅕms[8]까지는 0으로 clear 된 상태를 유지
	 intNums[9]=100;
	 //배열의 각 요소의 저장 값 읽기
	 ...
	 /*
	  * print()method는 ()안에 데이터  type에 따라 작동하는
	  * 방식이 다르다.
	  * 
	  *  print(변수):변수에 저장된 값을 console 에 출력
	  *  print(연산):연산을 먼저 수행하고 연산 결과를 console에 출력
	  *  print(method()):method()실행하고 return된 결과를 console에 출력
	  *  
	  *  print(객체,인스턴스):객체.toString(),인스턴스.toString()를 먼저 실행하고
	  *  return 문자열 결과를 Console에 출력
	  *  단,method()가 void type 인 경우는 오류가 난다.
	  *  
	  *  print(배열):배열.toString()을 먼저 실행하고
	  *  return된 기억장소 주소를 console 에 출력
	  */
